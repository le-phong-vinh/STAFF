...
public class InvoiceServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        // ✅ Xử lý thanh toán kèm phương thức
        if ("pay".equals(action)) {
            String idStr = request.getParameter("invoiceId");
            String paymentMethod = request.getParameter("paymentMethod");

            if (idStr == null || idStr.isEmpty() || paymentMethod == null || paymentMethod.isEmpty()) {
                response.sendRedirect("manageInvoice?error=invalid_input");
                return;
            }

            try {
                int id = Integer.parseInt(idStr);
                dao.updatePayment(id, paymentMethod); // cập nhật status + phương thức
                dao.updateInvoiceTotal(id); // cập nhật tổng
                response.sendRedirect("manageInvoice?action=detail&invoiceId=" + id);
                return;
            } catch (NumberFormatException e) {
